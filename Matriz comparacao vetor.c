...
int comparar_vetor(Spa_Mat* mat, int* vet){
    int i, j, valor, lin_vet;

    for(i = 0; i < mat->n_lin; i++){
            lin_vet = 1;

        for(j = 0; j < mat->n_col; j++){
                valor = buscar_pos(i, j, mat);
...
int main(){
    ...
    for(i = 0; i < lin; i++){
        for (j = 0; j < col; j++) {
            scanf("%d", &valor);
            trocar(valor, i, j, mat);
        }
    }

    vet = (int*)malloc (sizeof(int) * col);

    for(j = 0; j < col; j++)
        scanf("%d", &vet[j]);

    lin_vet = comparar_vetor(mat, vet);
    printf("%d\n", lin_vet);
