...
Spa_Mat* soma_matriz(Spa_Mat* m1, Spa_Mat* m2){
    int i, j, lin, col, soma;

    if(m1->n_lin > m2->n_lin){
        lin = m1->n_lin;
    }else{
        lin = m2->n_lin;
    }
    ...
    for(i = 0; i < lin; i++){
        for(j = 0; j < col; j++){
            soma = buscar_pos(i, j, m1) + buscar_pos(i, j, m2);
...
int main(){
    int i, j, item, lin1, col1, lin2, col2;

    scanf("%d %d", &lin1, &col1);
    Spa_Mat* m1 = criar(lin1, col1);
    ...
    Spa_Mat* resultado = soma_matriz(m1, m2);
